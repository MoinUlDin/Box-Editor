...
from Local_Scripts.GUI.initial_screen import myFrame


class CustomGraphicsView(QGraphicsView):
    sg_shortcut_saving_event = pyqtSignal(str)
    ...
    def keyPressEvent(self, event):
        ...
        if modifier == Qt.KeyboardModifier.ControlModifier:
            values = event.keyCombination().key()
            if values == 83:
                self.sg_shortcut_saving_event.emit("main_graphics")
                return
...
class MainWindow(QMainWindow):
    ...
    sg_image_opened_from_list = pyqtSignal(str)
    ...
    def __init__(self):
        ...
        # styling 
        self.scene.setBackgroundBrush(QColor(191, 191, 191))
        self.setStyleSheet("""
                           QMainWindow{
        ...
        self.scene.addWidget(self.myframe.get_frame())
        # =====================================================================================================
    ...
    def connect_other_modules(self):
        ...
        self.sg_image_opened_from_list.connect(self.rect_drawer.reset_all_verables)              # ----------> rect_drawer

        self.view.sg_shortcut_saving_event.connect(self.toolbar.shortcut_saving_event_handling)  # ----------> toolbar
    ...
    def open_image_from_list(self, caller ,file_name, index):
        if len(self.image_loader.list_images) > 1:
            ...
            if self.pixmap:
                self.load_image()
                self.sg_image_opened_from_list.emit('main')
    ...
    def load_image(self):
        if self.pixmap and not self.pixmap.isNull():
            ...
            # selecting the first rectangle in the scene
            self.rect_drawer.sidebar_selection_changes('main', 0)
            self.view.setFocus()
