...
class RectDrawer(QObject):
    ...
    def __init__(self):
        ...
        self.resizing_threshold = 5
        # for detecting clicks
        self.dragging_threshold = 15
    ...
    def finish_rect(self, view, scene, event):
        ...
        print(f'S: {self.click_starting_position}, E: {self.click_ending_position}')
    ...
    def resizing_selected_rect(self, position):
        if not self.selected_rect or not self.resizing_side:
            return
    ...
    def sidebar_selection_changes(self, _, index):
        if index < len(self.list_rect):
            if self.selected_rect:
                self.deselect_current_rect()
